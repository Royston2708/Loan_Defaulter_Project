...
W_1 = tf.Variable(tf.random_normal(shape=[26, 50]))
b_1 = tf.Variable(tf.random_normal(shape=[50]))
a1 = tf.sigmoid(tf.add(tf.matmul(X_data, W_1), b_1))

W_2 = tf.Variable(tf.random_normal(shape=[50, 10]))
b_2 = tf.Variable(tf.random_normal(shape=[10]))
a2 = tf.sigmoid(tf.add(tf.matmul(a1, W_2), b_2))

W_out = tf.Variable(tf.random_normal(shape=[10, 1]))
...
with tf.Session() as sess:
    ...
    print("Confusion Matrix is\n", confusion_matrix(Y_test, sess.run(answer, feed_dict={X_data: X_test, Y_data:Y_test})))
